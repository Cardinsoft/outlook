...
_CacheService.prototype.getDocumentCache = function () {
	//future releases;
	return this;
}
_CacheService.prototype.getScriptCache = function () {
	const storage = window.sessionStorage;
	return storage;
}
_CacheService.prototype.getUserCache = function () {
	//future releases;
	return this;
}
...
class ActionResponseBuilder extends _CardService {
	constructor() {
		super();
		this.navigation;
		this.notification;
...
async function performFetch(url) {
  ...
  var response = await UrlFetchApp.fetch(globalGateUrl,{'method':'post','payload':JSON.stringify(payload),'muteHttpExceptions':true,'contentType':'application/json'});
  ...
  return response;
	...
	response.then(function(result) {
		const response = new HTTPResponse(result.headers,result.content,result.code);
		resp = response;
		return resp;
	});
	
	response.catch(function(reject) {
		const response = new HTTPResponse(reject.headers,reject.content,reject.code);
		resp = response;
		return resp;
	});
...
var globalGateUrl = 'https://prod-76.westeurope.logic.azure.com:443/workflows/294389237fb346de8c9f868bc4ee95ba/triggers/manual/paths/invoke?api-version=2016-06-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=q-CnRc_vPtyk4zYgu1TKBDjUi_X929GCMZqvSq0Yky0';