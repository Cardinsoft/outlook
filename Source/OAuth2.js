...
function createOAuth2ResultUi(isAuthed, name, obj) {
  ...
  template.name = name; //access authorization parameters;   

  var errors = obj.error; //set authorization result;
  ...
  template.icon = '';
  template.logo = globalCardinIconUrl; //evaluate template and set parameters;

  var evaluated = template.evaluate();
  evaluated.setTitle(name + ' authorization result');
  evaluated.setHeight(500);
  evaluated.setWidth(200); //return HtmlOutput;
...
function authService(parameters) {
  ...
  var prompt = toBoolean(parameters.prompt); //access user properties, cache and lock services;

  var userStore = PropertiesService.getUserProperties();
  var userCache = CacheService.getUserCache();
  var userLock = LockService.getUserLock(); //set service parameters;
  ...
  service.setLock(userLock); //set optional parameters;

  if (hint) {
    service.setParam('login_hint', active);
  }
...
function callback(obj) {
  ...
  var parameters = obj.parameters; //create authorization service;

  var service = authService(parameter); //handle response and check if authorized;
  ...
  var name = parameter.serviceName + '_OAuth2Params'; //access storage and set partameters;
...
function revokeAuth(e) {
  var builder = CardService.newActionResponseBuilder(); //access parameters;

  var parameters = e.parameters; //create authorization service;

  var service = authService(parameters); //check if service has access and notify user;
