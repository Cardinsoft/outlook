...
let e_CacheService = function e_CacheService() {
  (0, _classCallCheck2.default)(this, e_CacheService);
...
e_CacheService.prototype.getScriptCache = function () {
  ...
  const session = window.sessionStorage; //create an instance of Cache with storage;
...
e_CacheService.prototype.getUserCache = function () {
  //access session storage;
  const local = window.localStorage; //create an instance of Cache with storage;
...
let Cache = function Cache(storage) {
...
Cache.prototype.get = function (key) {
  //access storage;
  const cache = this.storage; //access value by key;
...
Cache.prototype.getAll = function (keys) {
  //access storage;
  const cache = this.storage; //initialize result;
...
Cache.prototype.put = function (key, value) {
  //access storage;
  const cache = this.storage; //if value is an object, stringify;
